...
def register_session_id(session_id: str):
    """Register session in registry (best effort)"""
    try:
        session_registry = SCRIPT_DIR / 'session-registry.py'
        if session_registry.exists() and session_id:
            terminal_name = os.environ.get("CE_TERMINAL") or os.environ.get("HOSTNAME") or "terminal"
            subprocess.run(
                [sys.executable, str(session_registry), "register", terminal_name, "--id", session_id, "--quiet"],
                cwd=str(MEMORY_DIR),
                capture_output=True,
                text=True,
            )
...
def ensure_session_id() -> Optional[str]:
    """Ensure active/.session_id exists and is registered"""
...
""")
    ...
    # Ensure session id exists and register
    ensure_session_id()
        ...
        if result.returncode == 0:
            ...
            if not dest.exists():
                ...
                for f in ACTIVE_DIR.glob('*.md'):
                    if not f.name.startswith('TEMPLATE'):
                        f.unlink()
                # Update session registry (best effort)
                try:
                    session_id_file = ACTIVE_DIR / '.session_id'
                    ...
                    if session_registry.exists() and session_id:
                        subprocess.run(
                            [sys.executable, str(session_registry), "end", "--id", session_id, "--quiet"],
                            cwd=str(MEMORY_DIR),
                            capture_output=True,
                            text=True,
                        )
